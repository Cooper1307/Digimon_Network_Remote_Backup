# AIE Project Database: Phase 3 (Digital Monster Network) v1.0

This database serves as the single source of truth for all lore, entities, and systemic facts established or active during Phase 3.

## 1. Characters & Personas

| Name (CN) | Name (EN) | Identity | Species/Form | Status | Key Traits |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **万代 樱** | **Bandai Sakura** | Protagonist | Rosemon | Active | Academic researcher. Permanent data resident. |
| **枭** | **Owl** | Leader (Free Birds) | BlackWarGreymon | Active | Stoic "Ebon Hero." Seeks to isolate Digital World. |
| **亚当** | **Adam** | Guild Master | HerculesKabuterimon | Active | Cynical Information Broker. Paris-born. |
| **隼** | **Falcon** | Vanguard (Free Birds) | CresGarurumon | Active | Energetic, speed-focused attacker. |
| **鸽** | **Dove / Pigeon** | Defense (Free Birds) | MegaGargomon | Active | Kind-hearted (human form) vs. Berserker (combat). |
| **鹤** | **Heron / Crane** | Reserve (Free Birds) | Mastemon | Active | Cold, traumatized by sister's death. |
| **雀** | **Sparrow** | Former Support | Angewomon | Deceased | Owl's former partner, Dove/Heron's sibling. |
| **龙** | **Dragon** | Big Five Rep | Dorugoramon | Active | "Invincible King." China's representative. |
| **曼特** | **Metal** | Big Five Rep | GrandLocomon | Active | Russian military operative. |
| **玉** | **Jade** | Big Five Rep | BanchoStingmon | Active | Adam's sister. French vanguard. |
| **小王** | **Wang** | Disciple | Ouryumon | Active | Dragon's disciple. Decompiled. |
| **零** | **Zero** | Big Five Rep | Jesmon | Active | Silent lone wolf. CIA-modified. |

## 2. Locations & World-Building

| Term (CN) | Term (EN) | Category | Description | Logical Rule |
| :--- | :--- | :--- | :--- | :--- |
| **喝一杯酒吧** | **The 'Drink' Bar** | Guild / Hub | Central gathering place in Chuge area. | Neutral ground. |
| **自由鸟** | **Free Birds** | Raiding Group | Owl's independent group. | Goal: Sever Digital/Real link. |
| **自然庭院** | **Nature Court** | Guild Domain| Home to Snake and other nature-aligned entities. | Inner space maintenance ongoing. |
| **联合国五常攻略组**| **UN Big Five Raiders**| International | Elite state-sponsored teams. | Geopolitical rivalries. |

## 3. Technical & System Concepts

| Entity (CN) | Entity (EN) | Category | Lore Effect |
| :--- | :--- | :--- | :--- |
| **数码核** | **DigiCore** | System | The soul/engine of a Digimon. Critical damage is fatal. |
| **时之数码合金** | **Chromes Digizoid** | Material | Near-indestructible metal coating. |
| **反编译** | **Decompilation** | Condition | Irreversible loss of human logout capability. |
| **临时添补数据** | **Temp-Patch Data** | Item | Rare resource used to repair DigiCore/physical damage. |

---
*Last Updated: 2025-12-19 23:45:00 - Phase 3 INITIALIZED*
